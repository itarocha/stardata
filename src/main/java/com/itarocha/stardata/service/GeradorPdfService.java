    ...
    public static final String FONT_ASTRO = "src/main/resources/fonts/AstroDotBasic.ttf";
	 ...
	 public List<Interpretacao> createArquivo(boolean isTudo, MapaService servico, Mapa mapa, String texto) throws IOException {
		 ...
		 for(PlanetaPosicao pp : mapa.getPosicoesPlanetas()){
			 ...
			 SignoSolarEntity ss = servico.findSignoSolar("xx");
			 key = "O Signo Solar";
		 ...
		 for(PlanetaPosicao pp : mapa.getPosicoesPlanetas()){
			 ...
			 // Apresentação
			 PlanetaSignoEntity ps = servico.findPlanetaSigno(pp.getEnumPlaneta().getSigla(), pp.getEnumPlaneta().getSigla());
		 ...
		 key = "As Casas";
		 MapaCuspideEntity mc = servico.findCuspide("xx", 0);
		 if (mc != null) {
		...
		for(PlanetaPosicao pp : mapa.getPosicoesPlanetas()){
			//PlanetaPosicao pp = mapa.getPosicoesPlanetas().get(i);
			 ...
			 PlanetaCasaEntity pc = servico.findPlanetaCasa(pp.getEnumPlaneta().getSigla(), 0);
			 if (pc != null) {
		...
		for(ItemAspecto ia : mapa.getListaAspectos() ){
			...
			MapaPlanetaAspectoEntity a = servico.findAspecto(pA.getEnumPlaneta().getSigla(), pB.getEnumPlaneta().getSigla(), aspecto.getSigla() );
			 if (a != null) {
